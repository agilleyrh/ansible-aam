...
async def resolve_user(
    x_rh_user: str | None = Header(default=None, alias="x-rh-user"),
    x_rh_email: str | None = Header(default=None, alias="x-rh-email"),
    x_rh_roles: str | None = Header(default=None, alias="x-rh-roles"),
    x_rh_groups: str | None = Header(default=None, alias="x-rh-groups"),
    x_rh_identity: str | None = Header(default=None, alias="x-rh-identity"),
) -> UserContext:
    settings = get_settings()
    identity = _parse_identity_header(x_rh_identity)
    identity_user = identity.get("identity", {}).get("user", {})

    username = x_rh_user or identity_user.get("username")
    email = x_rh_email or identity_user.get("email")
    roles = _split_csv(x_rh_roles)
    groups = _split_csv(x_rh_groups)

    if not username and settings.environment == "development":
